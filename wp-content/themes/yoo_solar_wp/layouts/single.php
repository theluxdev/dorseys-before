...
<div id="system">
	...
	<?php if (have_posts()) : ?>
		...
		<article class="item" data-permalink="<?php the_permalink(); ?>">
			...
			<?php if($warp["config"]->get("single_view_featured_image", 1)) : ?>
				<?php if (has_post_thumbnail()) : ?>
					<?php
					$width = get_option('thumbnail_size_w'); //get the width of the thumbnail setting
					$height = get_option('thumbnail_size_h'); //get the height of the thumbnail setting
					?>
					<div class="featured-image"><?php the_post_thumbnail(array($width, $height), array('class' => 'size-auto')); ?></div>
				<?php endif; ?>
			<?php endif; ?>
			...
			<header>
				...
				<?php if($warp["config"]->get("single_view_meta", 1)) : ?>
					<p class="meta">
						<?php
